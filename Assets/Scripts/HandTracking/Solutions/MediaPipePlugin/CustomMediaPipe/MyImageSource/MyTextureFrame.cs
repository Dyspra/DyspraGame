...
#pragma warning restore IDE0065
  ...
  {
    // public class ReleaseEvent : UnityEvent<MyTextureFrame> { }

    private const string _TAG = nameof(TextureFrame);

    private static readonly GlobalInstanceTable<Guid, MyTextureFrame> _InstanceTable = new GlobalInstanceTable<Guid, MyTextureFrame>(100);
    /// <summary>
...
#pragma warning restore IDE1006
    ...
    private MyTextureFrame(Texture2D texture)
    {
      UnityEngine.Debug.Log("MyTextureFrame");
      _texture = texture;
      width = texture.width;
      ...
      UnityEngine.Debug.Log($"format: {format}");
      // OnRelease = new ReleaseEvent();
      _instanceId = Guid.NewGuid();
      UnityEngine.Debug.Log($"_instanceId: {_instanceId}");
      _InstanceTable.Add(_instanceId, this);
      UnityEngine.Debug.Log($"_InstanceTable: {_InstanceTable}");
    }

    public MyTextureFrame(int width, int height, TextureFormat format) : this(new Texture2D(width, height, format, false)) { }
    public MyTextureFrame(int width, int height) : this(width, height, TextureFormat.RGBA32) { }
