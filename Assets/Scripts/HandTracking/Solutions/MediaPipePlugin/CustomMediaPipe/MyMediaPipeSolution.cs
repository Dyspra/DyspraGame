...
namespace Mediapipe.Unity.Dyspra
{
  public class MyMediaPipeSolution : MyImageSourceSolution<MyMediaPipeGraph>
  {
    ...
    public Vector3 LeftHandPosition { get; private set; } = new Vector3(0, 0, 0);
    ...
    protected override IEnumerator WaitForNextValue()
    {
      ...
      if (handWorldLandmarks != null && handedness != null)
      {
      ...
      {
        Task.Run(() => ApplyHandPosition(handedness, handLandmarks, 0));
        Task.Run(() => ApplyHandPosition(handedness, handLandmarks, 1));
      }
      DrawNow(outputVideo);
    }
    ...
    private void ApplyHandLandmarks(List<ClassificationList> handedness, List<LandmarkList> handLandmarks, int handIndex)
    {
      ...
      if (handedness[handIndex].Classification.Count > 0 && handLandmarks[handIndex].Landmark.Count > 0)
      {
        ...
        if (handedness[handIndex].Classification[0].Index == 0 && handedness[handIndex].Classification[0].Score > 0.9f) // Left hand
        {
          LeftHandLandmarks = handLandmarksArray;
        }
        else if (handedness[handIndex].Classification[0].Index == 1 && handedness[handIndex].Classification[0].Score > 0.9f) // Right hand
        {
    ...
    private void ApplyHandPosition(List<ClassificationList> handedness, List<NormalizedLandmarkList> handLandmarks, int handIndex)
    {
      if (handedness[handIndex].Classification.Count > 0 && handLandmarks[handIndex].Landmark.Count > 0)
      {
        if (handedness[handIndex].Classification[0].Index == 0 && handedness[handIndex].Classification[0].Score > 0.9f) // Left hand
        {
          LeftHandPosition = new Vector3(handLandmarks[handIndex].Landmark[0].X, -handLandmarks[handIndex].Landmark[0].Y, -handLandmarks[handIndex].Landmark[0].Z);
        }
        else if (handedness[handIndex].Classification[0].Index == 1 && handedness[handIndex].Classification[0].Score > 0.9f) // Right hand
        {
          RightHandPosition = new Vector3(handLandmarks[handIndex].Landmark[0].X, -handLandmarks[handIndex].Landmark[0].Y, -handLandmarks[handIndex].Landmark[0].Z);
        }
      }
    }
